...
export function MapChart({
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (child) {
          const descriptor = child.type as unknown as MapDescriptor
          if (typeof descriptor === 'object') {
            const cached = getCachedMapSvg(descriptor.name, PACKAGE_VERSION)
            if (cached !== null) {
              if (!cancelled) setSvgMap(cached)
            } else {
              if (!cancelled) setIsLoading(true)
              const svg = await fetchAndCacheMapSvg(descriptor, PACKAGE_VERSION)
  ...
  // areaNameOnMap, or the map SVG itself (fetched asynchronously here) change;
  // useEffect always runs after the DOM is committed, so unlike Vue's watcher
  // there's no need for a separate "first render" workaround.
  useEffect(() => {
  ...
  return (
    <div className="v3mc-container" style={{ height: mapHeight, width: mapWidth }}>
      <div
        className="v3mc-tiny-loader-wrapper"
        style={{ display: isLoading ? 'flex' : 'none' }}>
        {loader ?? (
          <div className="v3mc-tiny-loader" style={{ borderTopColor: loaderColor }} />
        )}
      </div>
      <div
        id={containerId}
        ref={containerRef}
        className="v3mc-map"
        style={{ ...mapStyles, display: isLoading ? 'none' : undefined }}
