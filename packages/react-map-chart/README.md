# React Map Chart ![npm (scoped)](https://img.shields.io/npm/v/@arkn/react-map-chart)
...
```tsx
import { MapChart, AfricaMap, AsiaMap, BrazilMap, EgyptMap, EuropeMap, NorthAmericaMap } from '@arkn/react-map-chart'
...
> `<WorldMap />` (and every other map export) is never actually rendered — `MapChart` reads it back off the element to know which map to load. It's fine (and expected) for it to type-check as a component while not behaving like one.
